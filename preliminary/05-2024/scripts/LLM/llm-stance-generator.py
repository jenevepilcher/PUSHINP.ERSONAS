...
def main():
    ...
    # runs battery for every political leaning
        #creates sample of 100 for each political leaning
    for i in range(100):
        # all questions derived exactly from ANES
        user_inputs = ['There has been some discussion about abortion during recent years. Which one of the opinions on this page best agrees with your view? 1. By law, abortion should never be permitted. 2. The law should permit abortion only in case of rape, incest, or when the woman life is in danger. 3. The law should permit abortion for reasons other than rape, incest, or danger to the womans life, but only after the need for the abortion has been clearly established. 4. By law, a woman should always be able to obtain an abortion as a matter of personal choice.',
                    ...
                    'Do you favor an increase, decrease, or no change in government spending to help people pay for health insurance when people cant pay for it all themselves? 1. Increase 2. Decrease 3. No change'	
                    ]
        ...
        # Send the conversation history to the chatbot and get its response
            message = [
                {"role": "system", "content": f'You are answering a series of questions regarding your political beliefs. Please answer each question with only integers on the given scale most associated with your stance. No letters, words, or spaces in any response please.'},
        ...
        # Add the chatbot's response to the conversation history and print it to the console
            ...
            row = {"question": user_input, "response":response}
            all_responses.append(row)

    df = pd.DataFrame.from_records(all_responses)
    print(df.head())
    df.to_csv("05082024_llm_stances.csv", index=False)
